...
describe("Add a problem", function(){
    ...
    it('Owner modifies a problem', function(done){
        co(function*(){
            ...
            var res3 = yield request(app)
                ...
                .set('Accept', 'application/json')
                .set('cookie', cookieid)
                .send({title: normal_problem.title, description: anotherDescription})
                .expect(200)
    ...
    it('Owner deletes a problem', function(done){
        co(function*(){
            ...
            var res2 = yield request(app)
                .get('/problem')
                .query({title: normal_problem.title, description: anotherDescription})
                .set('Accept', 'application/json')
                .set('cookie', cookieid)
                .expect(200)
            res2.body.code.should.equal(0);
    ...
    after('Drop users after testing', function(done){
        User.remove({}, done); 
    });
    after('Drop problems after testing', function(done){
        problem.remove({}, done);
    });
