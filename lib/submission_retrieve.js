 ...
 * @apiSuccess {Object[]} submissions 提交文档列表(剔除 sumission.$.body 部分)
 */

function SubmissionRetrieve(req, res, next) {
  co(function * () {
    ...
    skip = parseInt(skip);
    let submissions =
        yield Submission.find(query, {body: 0}).limit(limit).skip(skip).exec();
